...
from models.siammae_pretrain.dataset_musicavqa import MusicavqaDataset
from models_siammae import SiamMAE
...
def train(config):
    ...
    dataset = MusicavqaDataset(config, model.image_processor)
    ...
    for epoch in range(config.train.start_epoch, config.train.epochs + 1):
        ...
        for index, (audio_feat, frame1_feats, frame2_feats) in enumerate(tqdm(pretrain_dataloader), start=1):
            audio_feat = audio_feat.cuda()
            frame1_feats = frame1_feats.cuda()
            frame2_feats = frame2_feats.cuda()
            with autocast():
                total_loss, loss_frame2_recon, loss_audio_recon = model(audio_feat, frame1_feats, frame2_feats)
