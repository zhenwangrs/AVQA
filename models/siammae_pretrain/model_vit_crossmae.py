...
from cross_attn_block import ViTMAECrossLayer
...
class ViTMAEEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, noise=None, apply_mask=True):
...
class ViTMAEModel(ViTMAEPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output, mask, ids_restore = self.embeddings(pixel_values, noise=noise, apply_mask=apply_mask, )
...
class ViTMAEDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_layers = nn.ModuleList(
            [ViTMAECrossLayer(decoder_config) for _ in range(config.decoder_num_hidden_layers)]
        )
    ...
    def forward(
        ...
        last_hidden_state = self.decoder_norm(hidden_states)

        # predictor projection
        logits = self.decoder_pred(last_hidden_state)

        # remove cls token
        logits = logits[:, 1:, :]

        if not return_dict:
            return tuple(v for v in [logits, last_hidden_state, all_self_attentions] if v is not None)
...
class ViTMAEForPreTraining(ViTMAEPreTrainedModel):
    ...
    def forward(
        self,
        pixel_values: Optional[torch.FloatTensor] = None,
        k: Optional[torch.FloatTensor] = None,
        v: Optional[torch.FloatTensor] = None,
        noise: Optional[torch.FloatTensor] = None,
        head_mask: Optional[torch.FloatTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        decoder_outputs = self.decoder(latent, k, v, ids_restore)
