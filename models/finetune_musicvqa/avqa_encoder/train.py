...
from dataset_mavqa_finetune import MavqaDataset
from models_avqa import FusVQAModel
...
def train():
    ...
    if config.train.epochs > 0:
        train_dataset = MavqaDataset(config, model.tokenizer, 'train')
        ...
        for epoch in range(start_epoch, config.train.epochs + 1):
            ...
            for index, (text_input_ids, text_attention_mask, audio_feats, frame_feats,
                        audio_feats_mean, frame_feats_mean, labels, question_types, question_ids) in enumerate(
                ...
                audio_feats_mean = audio_feats_mean.cuda()
                frame_feats_mean = frame_feats_mean.cuda()
                labels = labels.cuda()
                with autocast():
                    loss, logits = model(text_input_ids, text_attention_mask, labels,
                                         audio_feats_mean, frame_feats_mean, audio_feats, frame_feats)
...
def test(model, config, split='test'):
    ...
    test_dataset = MavqaDataset(config, model.tokenizer, split)
...
if __name__ == '__main__':
    logging.basicConfig(filename='log_multi_llm.log', level=logging.INFO,
