...
def train():
    ...
    if config.train.epochs > 0:
        train_dataset = MavqaDataset_online(config, model.tokenizer, model.image_processor, 'train', fix_select=True)
...
def test(model, config, split='test'):
    ...
    test_dataset = MavqaDataset_online(config, model.tokenizer, model.image_processor, split, fix_select=True)
