...
from dataset_avqa_lstm import AVQADataset
...
def train():
    ...
    if config.train.training_mode:
        train_dataset = AVQADataset(config, model.tokenizer, model.image_processor, 'train')
        ...
        for epoch in range(start_epoch, config.train.epochs + 1):
            ...
            for index, (text_input_ids, text_attention_mask, audio_feats, frame_feats,
                        labels, answer_index, question_ids) in enumerate(
...
def test(model, config, split='test'):
    ...
    with torch.no_grad():
        ...
        for index, (text_input_ids, text_attention_mask, audio_feats, frame_feats,
            ...
            for qid, pred_score, label, ans_index in zip(question_ids, pred_score_list, labels, answer_index):
                if qid not in answer_record:
                    answer_record[qid] = {
                        'pred_ans_score': [0] * 4,
                        'correct_ans_index': -1,
                    }
                answer_record[qid]['pred_ans_score'][ans_index] = pred_score[1]
                if label == 1:
                    answer_record[qid]['correct_ans_index'] = ans_index
